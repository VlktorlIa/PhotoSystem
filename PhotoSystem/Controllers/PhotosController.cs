...

{
...

    {
...

        {
            ...
            // повертає назад на ту сторінку, де натиснули
        ...
        // Новий метод для перемикання лайка (Toggle)
...

        {
            ...
            // Шукає, чи є вже лайк від цього користувача на цей пост

            var existingLike = await _context.PhotoLikes

                .FirstOrDefaultAsync(l => l.PhotoPostId == postId && l.UserId == userId);
...

            {
                ...
                // Якщо лайк є видаляє його (Unlike)
...

            {
                ...
                // Якщо лайка немає створює новий (Like)
            ...
            // Повертає користувача туди, звідки він прийшов (на стрічку або в профіль)
...

        {
            ...
            // Завантажує сам пост автора та лайки
            ...
            // Завантажує коментарі окремо через контекст і передає у View через ViewBag

            ViewBag.Comments = await _context.Comments

                .Where(c => c.PhotoPostId == id)
        ...
        public async Task<IActionResult> GetIncrementalFeed(int page = 1, int pageSize = 5)

        {
...

            {
                ...
                return NoContent(); // Повертає 204 (порожньо)

            }



            // Повертає PartialView. MVC сам знайде _PostCard.cshtml

            // Оскільки треба повернути список карток просто рендеримо їх одну за одною

            return PartialView("_FeedChunk", posts);
